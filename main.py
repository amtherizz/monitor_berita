...
def hapus_duplikat(kalimat):
    kata_kata = kalimat.split()  # pecah jadi list kata
    hasil = []
    for kata in kata_kata:
        if kata not in hasil:  # cek apakah sudah ada di hasil
            hasil.append(kata)
    # print(hasil)
    return " ".join(hasil)
...
def index():
    ...
    positif_text = ' '.join([bersihkan_kalimat(x) for x in df[df['sentiment'] == 'positif']['isi isu'] if x])
    positif_text = bersihkan_kalimat(positif_text)
    
    common_words = Counter(re.findall(r'\b[a-zA-Z]{2,}\b', positif_text)).most_common(20)
    df_kata = pd.DataFrame(common_words, columns=['kata', 'jumlah'])
    wcp = False
    if positif_text:
        wcp = px.bar(df_kata.sort_values('jumlah'), 
                  x='jumlah', y='kata', 
                  orientation='h',
                  title='20 Kata Paling Sering Muncul dalam sentiment positif',
                  labels={'jumlah': 'Frekuensi', 'kata': 'Kata'})

    # WordCloud Negatif
    negatif_text = ' '.join([bersihkan_kalimat(x) for x in df[df['sentiment'] == 'negatif']['isi isu'] if x])
    negatif_text = bersihkan_kalimat(negatif_text)
    wcn = False
    common_words = Counter(re.findall(r'\b[a-zA-Z]{2,}\b', negatif_text),).most_common(20)
    df_kata = pd.DataFrame(common_words, columns=['kata', 'jumlah'])
    if negatif_text:
        wcn = px.bar(df_kata.sort_values('jumlah'), 
                  x='jumlah', y='kata', 
                  orientation='h',
                  title='20 Kata Paling Sering Muncul dalam sentiment negatif',
                  labels={'jumlah': 'Frekuensi', 'kata': 'Kata'})
    ...
    return render_template('index.html', 
                           ...
                           wc_p=wcp.to_html(full_html=False),
                           wc_n=wcn.to_html(full_html=False),
                           keywords=['badan informasi geospasial','infomasi geospasial tematik','BIG'],
